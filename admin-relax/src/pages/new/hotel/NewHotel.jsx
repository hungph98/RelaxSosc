...
const NewHotel = ({inputs}) => {
    ...
    const {data, loading, error} = useFetch(`http://localhost:8080/api/v1/rooms`);
    ...
    const handleClick = async (e) => {
        ...
        try {
            const list = await Promise.all(Object.values(files).map(async (file) => {
                ...
                const uploadRes = await axios.post("https://api.cloudinary.com/v1_1/duwoaaglj/image/upload", data);
            ...
            await axios.post("http://localhost:8080/api/v1/hotels", newHotel)
        } catch (err) {
